...
# slice 3D TI and transform it into 2D dataset
def sliceTI(TI, N_X, N_Y, dx=5, dy=5):
    images = []
    nx, ny, nz = TI.shape
    for k in range(nz):
        for i in range(0, nx-N_X, dx):
            for j in range(0, ny-N_Y, dy):
                images.append(TI[i:i+N_X,j:j+N_Y,k])
